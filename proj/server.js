...
async function sourceContext(projectId) {
  ...
  const usable = files.filter(file => file.textContent);
...
async function runAssetJob(projectId, targets, context) {
  const prompt = `从资料中提取并整理以下资产类别：${targets.join('、')}。
输出格式：
{"assets":[{"type":"character|location|prop|style|sound|music","name":"资产名","description":"可直接用于制作的具体描述","tags":["标签"],"visualAnchor":"视觉锚点或声音锚点","continuity":"连续性要求","sourceRefs":["来源"],"confidence":"high|medium|low"}]}
同名资产合并，不能凭空新增主要人物。
资料如下：
${context}`;
  const data = parseAIJson(await callAI(FILM_SYSTEM, prompt));
  ...
  for (const item of assets) {
    ...
    await Store.put({
      id: `asset_${stable}`, projectId, kind: 'asset', subtype: item.type,
  ...
  try {
    ...
    for (const file of req.files || []) {
      ...
      const record = {
        ...
        name: file.originalname, status: text || file.mimetype.startsWith('image/') ? 'parsed' : 'stored',
        data: {
